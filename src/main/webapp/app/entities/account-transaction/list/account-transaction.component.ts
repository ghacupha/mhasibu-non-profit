...
@Component({
  selector: 'jhi-account-transaction',
...
export class AccountTransactionComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.accountTransactionService
      ...
      .subscribe(
        (res: HttpResponse<IAccountTransaction[]>) => {
  ...
  trackId(index: number, item: IAccountTransaction): number {
    return item.id!;
  }
  ...
  delete(accountTransaction: IAccountTransaction): void {
    const modalRef = this.modalService.open(AccountTransactionDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IAccountTransaction[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
